export const projects = [
  {
    id: "postly",
    title: "Postly Dev Tool",
    subtitle: "Convert CSV, JSON, and XML in one click",
    year: "2026",
    role: "Frontend Developer",
    stack: "React, Next.js, Firebase, ",
    image: "/postly.png",
    live: "https://postly-web-coral.vercel.app",
    repo: "https://github.com/aaswne/postly-web",

    overview:
      "Postly is a developer utility tool designed to simplify data transformation between CSV, JSON, and XML formats.",

    problem:
      "Developers often use multiple tools to convert data formats, making workflows slower and less efficient.",

    solution:
      "A minimal single-page tool with instant conversion and a clean user experience.",

    gallery: ["/postly-1.png", "/postly-2.png"],
  },

  {
    id: "samsara",
    title: "Samsara AI",
    subtitle: "AI chat application powered by Gemini",
    year: "2025",
    role: "Frontend Developer",
    stack: "React, Firebase, Gemini API",
    image: "/samsara.png",
    live: "#",
    repo: "#",

    overview:
      "Samsara AI is an AI-powered chat application focused on speed and clean interaction design.",

    problem:
      "Most AI chat interfaces feel cluttered and overwhelming for users.",

    solution:
      "Designed a distraction-free interface with fast response rendering and simple conversation flow.",

    gallery: ["/samsara-1.png", "/samsara-2.png"],
  },

  {
    id: "netflix",
    title: "Netflix Clone",
    subtitle: "Streaming platform inspired by Netflix",
    year: "2025",
    role: "Frontend Developer",
    stack: "React, TMDB API",
    image: "/netflix.png",
    live: "#",
    repo: "#",

    overview:
      "A modern Netflix-inspired streaming UI with responsive layouts and movie browsing.",

    problem:
      "Learning how large-scale streaming interfaces manage layouts and dynamic content.",

    solution:
      "Built reusable UI sections and dynamic movie rendering using APIs.",

    gallery: ["/netflix-1.png", "/netflix-2.png"],
  },

  {
    id: "olx",
    title: "Marketplace App",
    subtitle: "Marketplace platform inspired by OLX India",
    year: "2025",
    role: "Frontend Developer",
    stack: "React, Firebase",
    image: "/olx.png",
    live: "#",
    repo: "#",

    overview:
      "A marketplace application where users can browse and post products.",

    problem:
      "Understanding marketplace UI systems and product listing structures.",

    solution:
      "Created a responsive product-based UI with clean navigation and reusable components.",

    gallery: ["/olx-1.png", "/olx-2.png"],
  },
];